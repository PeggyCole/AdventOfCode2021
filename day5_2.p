...
procedure readFile:
  ...
  define variable viFrom2    as integer   no-undo.
  define variable viTo2      as integer   no-undo.
  define variable viTeller2 as integer   no-undo.
  ...
  repeat on error undo, leave on endkey undo, leave:
    ...
    do:
     ...
     blockTeller:   
      do viTeller = viFrom to viTo:
        if viFrom2 < viTo2 and viTeller2 > viTo2 then 
          leave blockTeller.
        
        if viFrom2 > viTo2 and viTeller2 < viTo2 then 
          leave blockTeller.
        
        if viFrom2 = viTo2 then
          
        run FillTT (viTeller, viTeller2).               
        
        if viFrom2 < viTo2 then viTeller2 = viTeller2 + 1.
        if viFrom2 > viTo2 then viTeller2 = viTeller2 - 1.
